	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < arr.length; i++) {
			if (arr[i] instanceof Circular_saws) {
				...
				System.out.println("Дрель " + arr[i].getName() + " " +arr[i].getCount() + " руб " +arr[i].getQuantity() + " шт " + ((Electro_tools) arr[i]).getPower_consumption() + " ватт и максимальный диаметр сверла " + ((Drill) arr[i]).getDiameter() + " мм");
				if (max < ((Electro_tools) arr[i]).getPower_consumption()) {
					max = ((Electro_tools) arr[i]).getPower_consumption();
				}
			}
				...
				System.out.println("Ноутбук " + arr[i].getName() + " " +arr[i].getCount() + " руб " +arr[i].getQuantity() + " шт " + ((Computer) arr[i]).getMemory() + " GB " + ((Computer) arr[i]).getHard_disk_space() + " TB " + ((Computer) arr[i]).getCPU_cores() + " CPU " + ((Laptops) arr[i]).getDiagonal() + " дюймов срок службы " + ((Laptops) arr[i]).getTime() + " год(а)");
